...
impl<'a> Scanner<'a> {
    pub fn new(source: &'a str, file: String, catch_comments: bool) -> Scanner {
        Scanner {
            ...
            match char {
                ...
                "-" => {
                    if let Some(&(_, ">")) = self.chars.peek() {
                        self.chars.next();
                        self.push(Token::minus(span.two_char()))
                    } else {
                ...
                "." => {
                    if let Some(&(_, ".")) = self.chars.peek() {
                        ...
                        if let Some(&(_, "=")) = self.chars.peek() {
                            self.chars.next();
                            self.push(Token::range(span.three_char()))
                        } else {
                ...
                "=" => {
                    if let Some(&(_, "=")) = self.chars.peek() {
                        ...
                        self.push(Token::declaration(span))
                    }
                }
                "<" => {
                    if let Some(&(_, "=")) = self.chars.peek() {
    ...
    fn push(&mut self, token: Token) {
        ...
        match token.kind {
            ...
            {
                self.is_beginning_of_line = false;
                for _ in 0..=(self.previous_indentation
                    - self.current_indentation / self.unit_indentation)
                {
                    self.tokens.push(Token::new(
                        TokenType::Dedent,
                        Span::from_scanner(self, self.position),
            ...
            Identifier | Constant | Wildcard | CharLiteral => {
                self.position = token.span.position + 1;
    ...
    fn constant(&mut self, first_char: &str) {
        ...
        loop {
            match self.chars.peek() {
                ...
                Some(&(index, _)) => {
                    return self.push(Token::constant(Span::from_scanner(self, index), content))
                }
                None => {
